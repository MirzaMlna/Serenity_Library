...
class IslamAddView extends StatefulWidget {
  const IslamAddView({Key? key}) : super(key: key);

  @override
  State<IslamAddView> createState() => _IslamAddViewState();
}

class _IslamAddViewState extends State<IslamAddView> {
  ...
  Future<void> _addEbookData() async {
    await IslamEBookSQLHelper.createIslamEBook(_eBookTitleController.text,
        _eBookLinkController.text, _eBookCoverController.text);
    _refreshData();
  }

  Future<void> _updateEbookData(int id) async {
    await IslamEBookSQLHelper.updateIslamEBook(id, _eBookTitleController.text,
        _eBookLinkController.text, _eBookCoverController.text);
    _refreshData();
  }

  Future<void> _deleteEbookData(int id) async {
    await IslamEBookSQLHelper.deleteIslamEBook(id);
  ...
  void showBottomSheet(int? id) async {
    if (id != null) {
      ...
      _eBookTitleController.text = existingData['islamEBookTitle'];
      _eBookLinkController.text = existingData['islamEBookPdf'];
      _eBookCoverController.text = existingData['islamEBookCover'];
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add Islam E-Book')),
                    ...
                    itemBuilder: (context, index) => SizedBox(
                          child: Column(
                            children: [
                              ...
                              Row(
                                children: [
                                  Text(_allEbookData[index]['islamEBookTitle']),
                                  const SizedBox(
                                    width: 10,
                                  )
                                ],
                  ...
                  itemBuilder: (context, index) => Column(
                    children: [
                      InkWell(
                        ...
                        onTap: () async {
                          final url = _allEbookData[index]['islamEBookPdf'];
                        ...
                        child: Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(10),
                            child: Image.network(
                              _allEbookData[index]['islamEBookCover'],
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
