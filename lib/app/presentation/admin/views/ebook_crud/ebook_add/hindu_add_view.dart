...
class HinduAddView extends StatefulWidget {
  const HinduAddView({Key? key}) : super(key: key);

  @override
  State<HinduAddView> createState() => _HinduAddViewState();
}

class _HinduAddViewState extends State<HinduAddView> {
  ...
  Future<void> _addEbookData() async {
    await HinduEBookSQLHelper.createhinduEBook(_eBookTitleController.text,
        _eBookLinkController.text, _eBookCoverController.text);
    _refreshData();
  }

  Future<void> _updateEbookData(int id) async {
    await HinduEBookSQLHelper.updatehinduEBook(id, _eBookTitleController.text,
        _eBookLinkController.text, _eBookCoverController.text);
    _refreshData();
  }

  Future<void> _deleteEbookData(int id) async {
    await HinduEBookSQLHelper.deletehinduEBook(id);
  ...
  void showBottomSheet(int? id) async {
    if (id != null) {
      ...
      _eBookTitleController.text = existingData['hinduEBookTitle'];
      _eBookLinkController.text = existingData['hinduEBookPdf'];
      _eBookCoverController.text = existingData['hinduEBookCover'];
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add hindu E-Book')),
                    ...
                    itemBuilder: (context, index) => SizedBox(
                          child: Column(
                            children: [
                              ...
                              Row(
                                children: [
                                  Text(_allEbookData[index]['hinduEBookTitle']),
                                  const SizedBox(
                                    width: 10,
                                  )
                                ],
                  ...
                  itemBuilder: (context, index) => Column(
                    children: [
                      InkWell(
                        ...
                        onTap: () async {
                          final url = _allEbookData[index]['hinduEBookPdf'];
                        ...
                        child: Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(10),
                            child: Image.network(
                              _allEbookData[index]['hinduEBookCover'],
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
