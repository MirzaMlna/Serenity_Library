...
class KristenAddView extends StatefulWidget {
  const KristenAddView({Key? key}) : super(key: key);

  @override
  State<KristenAddView> createState() => _KristenAddViewState();
}

class _KristenAddViewState extends State<KristenAddView> {
  ...
  Future<void> _addEbookData() async {
    await KristenEBookSQLHelper.createkristenEBook(_eBookTitleController.text,
        _eBookLinkController.text, _eBookCoverController.text);
    _refreshData();
  }

  Future<void> _updateEbookData(int id) async {
    await KristenEBookSQLHelper.updatekristenEBook(
        id,
  ...
  Future<void> _deleteEbookData(int id) async {
    await KristenEBookSQLHelper.deletekristenEBook(id);
  ...
  void showBottomSheet(int? id) async {
    if (id != null) {
      ...
      _eBookTitleController.text = existingData['kristenEBookTitle'];
      _eBookLinkController.text = existingData['kristenEBookPdf'];
      _eBookCoverController.text = existingData['kristenEBookCover'];
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Add kristen E-Book')),
                  ...
                  itemBuilder: (context, index) => Column(
                    children: [
                      InkWell(
                        ...
                        onTap: () async {
                          final url = _allEbookData[index]['kristenEBookPdf'];
                        ...
                        child: Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(10),
                            child: Image.network(
                              _allEbookData[index]['kristenEBookCover'],
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
                      ),
                      Text(_allEbookData[index]['kristenEBookTitle']),
                    ],
                  ),
                ),
              ),
